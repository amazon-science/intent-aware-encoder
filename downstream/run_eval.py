...
class ProtoNet(nn.Module):
    def __init__(self, encoder, metric="euclidean", label_fn=None, zero_shot=False):
        ...
        # self.role_labeler = IntentRoleLabeler.from_params(Params.from_file('config/tagger-intent-role-labeler.jsonnet'))
...
def test_proto(
    ...
    protonet = ProtoNet(encoder=bert, 
                        ...
                        zero_shot=zero_shot)
    if load_ckpt:
        protonet.load_state_dict(torch.load(ckpt_path))
...
def add_args():
    ...
    parser.add_argument("--cv", default='01,02,03,04,05')
    ...
    if args.load_ckpt and args.ckpt_path == None:
        raise NotImplementedError
...
def main(args):
    ...
    splits = args.cv.split(",")
    for split in splits:
        set_seeds(args.seed)
        if ckpt_path is not None:
            args.ckpt_path = ckpt_path + f"/{split}/encoder.pkl"
        args.test_path = test_path + f"/{split}/test.jsonl"
        args.output_path = output_path + f"/{split}"
